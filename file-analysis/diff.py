...
for i in range(len(md1)):
    one = md1[i]
    two = md2[i]
    print('mot %2d pos %4d %s  %4d %s' % (i+1, one['position'], one['side'], two['position'], two['side']), end=' ')
...
for i in range(len(b1lst)):
    ...
    print('0x%s   0x%02X   0x%02X  (%3d)  (%3d)' % (lt[0], lval, rval, lval, rval), end=' ')
    if lval != rval:
        print('<----')
    else:
        print()
